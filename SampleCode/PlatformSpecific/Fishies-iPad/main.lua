-- Abstract: Fishies sample app for iPad
--
-- Date: August 19, 2010
--
-- Version: 1.2
...
--
-- Demonstrates: 
-- 		Demonstrates graphics and moving objects (fishes) that are "touchable".
--  	Also demonstrates receiving Orientation events and rotating
--  	objects in response.
--		This code does not use build.settings for auto rotation.
--
...
--
-- Update History:
--	1.1		Fixed Landscape problem (blank background when started in Landscape mode)
--		 	Note that is version only works on iPad ios 3.2 because of unique Orientation events
-- 	1.2 	Changed to use new audio API, fixed white background problem
-- Comments: 
--
...
local curOrientation = "portrait"		-- **v1.1
local  isResumed = false				-- **v1.1
...
local bounceAnimation = {
	container = display.newRect( centerX, centerY, display.viewableContentWidth, display.viewableContentHeight ),
	reflectX = true,
}

local backgroundPortrait = display.newImage( "Default.png", centerX, centerY, true )
local backgroundLandscape = display.newImage( "Default-Landscape.png", centerX, centerY, true )

backgroundLandscape.isVisible = false
local background = backgroundPortrait

local soundID = audio.loadSound( "bubble_strong.caf" )
...
local backgroundOrientation = function( event )
	...
	-- Handle starting/resuming in Landscape mode **v1.1
	-- Ignores the first Orientation event after applicatonResume
	--
	if (curOrientation == event.type) or isResumed then
		event.delta = 0
		isResumed = false
	end
	...
	if ( delta ~= 0 ) then
		...
		curOrientation = event.type		-- **v1.1: save the current orientation
...
-- Ignore the first Orientation Event after resume (only works on iPad)
-----------------------------------------------------------------------
--
